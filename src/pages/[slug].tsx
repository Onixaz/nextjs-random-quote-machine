...
export async function getStaticProps({ params: { slug } }: Params) {
  const categoryRes = await fetch(`http://mldarbai.paju.tech/wp-json/wp/v2/pages?slug=${slug}`)

  const found = await categoryRes.json()

  const imagesRes = await fetch(`http://mldarbai.paju.tech/wp-json/wp/v2/media`)
...
export async function getStaticPaths() {
  const res = await fetch(`http://mldarbai.paju.tech/wp-json/wp/v2/pages?status=publish`)
